...
class pdf_sponge extends ModelePDFFactures
{
		...
		if ($conf->facture->multidir_output[$conf->entity]) {
			...
			if (file_exists($dir)) {
				...
				if ($qrcodestring) {
					...
					if (getDolGlobalString('INVOICE_ADD_EPC_QR_CODE') == '1' && (empty($object->mode_reglement_code) || $object->mode_reglement_code == 'VIR')) {
						if ($object->fk_account > 0 || $object->fk_bank > 0 || getDolGlobalInt('FACTURE_RIB_NUMBER')) {
							...
							$pdf->MultiCell(40, 3, $langs->transnoentitiesnoconv("INVOICE_ADD_EPC_QR_CODEPay"), 0, 'L', false);
						}
					}
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					if (defined('SUBTOTALS_SPECIAL_CODE') && $object->lines[$i]->special_code == SUBTOTALS_SPECIAL_CODE) {
					...
					// VAT Rate
					if ($this->getColumnStatus('vat') && (!defined('SUBTOTALS_SPECIAL_CODE') || $object->lines[$i]->special_code != SUBTOTALS_SPECIAL_CODE)) {
						$vat_rate = pdf_getlinevatrate($object, $i, $outputlangs, $hidedetails);
						$this->printStdColumnContent($pdf, $curY, 'vat', $vat_rate);
					}

					// Unit price before discount
					if ($this->getColumnStatus('subprice') && (!defined('SUBTOTALS_SPECIAL_CODE') || $object->lines[$i]->special_code != SUBTOTALS_SPECIAL_CODE) && isset($pdf_sub_options['titleshowuponpdf'])) {
					...
					// Quantity
					// Enough for 6 chars
					if ($this->getColumnStatus('qty') && (!defined('SUBTOTALS_SPECIAL_CODE') || $object->lines[$i]->special_code != SUBTOTALS_SPECIAL_CODE)) {
						$qty = pdf_getlineqty($object, $i, $outputlangs, $hidedetails);
						$this->printStdColumnContent($pdf, $curY, 'qty', $qty);
					}

					// Situation progress
					if ($this->getColumnStatus('progress') && (!defined('SUBTOTALS_SPECIAL_CODE') || $object->lines[$i]->special_code != SUBTOTALS_SPECIAL_CODE)) {
						$progress = pdf_getlineprogress($object, $i, $outputlangs, $hidedetails);
						$this->printStdColumnContent($pdf, $curY, 'progress', $progress);
					}

					// Unit
					if ($this->getColumnStatus('unit') && (!defined('SUBTOTALS_SPECIAL_CODE') || $object->lines[$i]->special_code != SUBTOTALS_SPECIAL_CODE)) {
						$unit = pdf_getlineunit($object, $i, $outputlangs, $hidedetails);
						$this->printStdColumnContent($pdf, $curY, 'unit', $unit);
					}

					// Discount on line
					if ($this->getColumnStatus('discount') && $object->lines[$i]->remise_percent && (!defined('SUBTOTALS_SPECIAL_CODE') || $object->lines[$i]->special_code != SUBTOTALS_SPECIAL_CODE)) {
					...
					if ($this->getColumnStatus('totalexcltax')) {
						if ((!defined('SUBTOTALS_SPECIAL_CODE') || $object->lines[$i]->special_code != SUBTOTALS_SPECIAL_CODE) && isset($pdf_sub_options['titleshowtotalexludingvatonpdf'])) {
				...
				for ($i = $pageposbeforeprintlines; $i <= $drawTabNumbPage; $i++) {
					...
					$this->_pagefoot($pdf, $object, $outputlangs, $hideFreeText, $this->getHeightForQRInvoice($pdf->getPage(), $object, $outputlangs));
	...
	protected function drawInfoTable(&$pdf, $object, $posy, $outputlangs, $outputlangsbis)
	{
		...
		if (isModEnabled('incoterm')) {
			...
			if ($desc_incoterms) {
				$pdf->SetFont('', '', $default_font_size - 1);
				$pdf->writeHTMLCell($this->page_largeur - $this->marge_gauche - $this->marge_droite, 3, $this->marge_gauche, $posy, dol_htmlentitiesbr($desc_incoterms), 0, 1);
				$nexY = $pdf->GetY();
				$height_incoterms = $nexY - $posy;

				// Rect takes a length in 3rd parameter
				$pdf->SetDrawColor(192, 192, 192);
				$pdf->RoundedRect($this->marge_gauche, $posy - 1, $this->page_largeur - $this->marge_gauche - $this->marge_droite, $height_incoterms + 3, $this->corner_radius, '1234', 'D');
		...
		if ($notetoshow) {
			$pdf->SetFont('', '', $default_font_size - 1);
			$pdf->writeHTMLCell($this->page_largeur - $this->marge_gauche - $this->marge_droite, 3, $this->marge_gauche, $posy, dol_htmlentitiesbr($notetoshow), 0, 1);
			$nexY = $pdf->GetY();
			$height_note = $nexY - $posy;

			// Rect takes a length in 3rd parameter
			$pdf->SetDrawColor(192, 192, 192);
			$pdf->RoundedRect($this->marge_gauche, $posy - 1, $this->page_largeur - $this->marge_gauche - $this->marge_droite, $height_note + 3, $this->corner_radius, '1234', 'D');
	...
	protected function drawTotalTable(&$pdf, $object, $deja_regle, $posy, $outputlangs, $outputlangsbis)
	{
		...
		if (!empty($TPreviousIncoice)) {
			...
			foreach ($TPreviousIncoice as &$fac) {
				...
				$pdf->MultiCell($largcol2, $tab2_hl, $displayAmount, 0, 'R', true);
		...
		if (getDolGlobalInt('PDF_INVOICE_SHOW_VAT_ANALYSIS')) {
			...
			$pdf->SetFillColor(255, 255, 255);
			$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
			$pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("TotalVAT"), 0, 'L', true);

			$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
			$pdf->MultiCell($largcol2, $tab2_hl, price($sign * $object->total_tva), 0, 'R', true);
		}
		...
		if (!getDolGlobalString('MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT')) {
			...
			if (getDolGlobalString('MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_IFNULL') && $tvaisnull) {
				...
				if (price2num($object->revenuestamp, 'MT') != 0) {
					$index++;
					$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
					$pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("RevenueStamp").(is_object($outputlangsbis) ? ' / '.$outputlangsbis->transnoentities("RevenueStamp", $mysoc->country_code) : ''), $useborder, 'L', true);

					$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
					$pdf->MultiCell($largcol2, $tab2_hl, price($sign * $object->revenuestamp), $useborder, 'R', true);
				}
				...
				if ($object->displayRetainedWarranty()) {
					...
					$index++;
					$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
					$pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("ToPayOn", dol_print_date($object->date_lim_reglement, 'day')), $useborder, 'L', true);

					$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
					$pdf->MultiCell($largcol2, $tab2_hl, price($billedWithRetainedWarranty), $useborder, 'R', true);
					...
					$pdf->MultiCell($col2x - $col1x, $tab2_hl, $retainedWarrantyToPayOn, $useborder, 'L', true);
					$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
					$pdf->MultiCell($largcol2, $tab2_hl, price($retainedWarranty), $useborder, 'R', true);
				}
			}
		}
		...
		if (!isModEnabled("multicurrency") || $object->multicurrency_tx == 1 || getDolGlobalInt('MULTICURRENCY_SHOW_ALSO_MAIN_CURRENCY_ON_PDF') == 0) {
			...
			$creditnoteamount_origin = $object->getSumCreditNotesUsed(0); // Warning, this also include excess received
			$depositsamount_origin = $object->getSumDepositsUsed(0);
			$resteapayer_origin = price2num($total_ttc_origin - $deja_regle_origin - $creditnoteamount_origin - $depositsamount_origin, 'MT');
		}
