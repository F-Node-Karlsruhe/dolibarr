...
function pdf_getlinedesc($object, $i, $outputlangs, $hideref = 0, $hidedesc = 0, $issupplierline = 0)
{
	...
	if ($idprod && isModEnabled('mrp')) {
		...
		if (!empty($object->linkedObjectsIds['mo']) || !empty($object->linkedObjects['mo'])) {
			...
			if (!empty($manufactureOrders)) {
				$materialsUsed = array();
				
				foreach ($manufactureOrders as $mo) {
					
					// Check if this MO is for the current product
					if ($mo->fk_product == $idprod) {
						
						// Get consumed materials for this MO
						$consumedMaterials = $mo->fetchLinesLinked('consumed');
						
						foreach ($consumedMaterials as $material) {
							
							if (!empty($material['batch'])) {
								$batchKey = $material['batch'];
								
								if (!isset($materialsUsed[$batchKey])) {
									$materialsUsed[$batchKey] = array(
								...
								$materialsUsed[$batchKey]['qty'] += $material['qty'];
								$materialsUsed[$batchKey]['mo_refs'][] = $mo->ref;
							}
						}
					}
				}
				...
				if (!empty($materialsUsed)) {
					$materialsString = 'Materials Used: ';
					...
					$materialsString .= implode('; ', $materialDetails);
					
					// Add to description
					$libelleproduitservice = dol_concatdesc($libelleproduitservice, $materialsString);
				} 
			} 
		} 
	}
...
function pdf_getLinkedObjects(&$object, $outputlangs)
{
	...
	foreach ($object->linkedObjects as $objecttype => $objects) {
		...
		} elseif ($objecttype == 'commande' || $objecttype == 'order_supplier') {
